...
const faqItems = [
  {
    id: 1,
    icon: <HelpCircle size={20} className="text-turquoise" />,
  ...
  {
    id: 2,
    icon: <ShieldCheck size={20} className="text-turquoise" />,
  ...
  {
    id: 3,
    icon: <FileText size={20} className="text-turquoise" />,
  ...
  return (
    <motion.main className="section-spacing mx-auto flex max-w-3xl flex-col items-center justify-start font-hind text-white">
      ...
      >
        ...
        <h1 className="text-4xl font-bold text-white">FAQ – Questions fréquentes</h1>
      </motion.header>
      ...
      <section>
        {faqItems.map((item, i) => (
          <motion.div
            key={item.id}
            className="bg-eerie cursor-pointer rounded-lg border border-turquoise/30 p-4 shadow-md"
            onClick={() => toggle(i)}
            initial={{ opacity: 0, y: 20 }}
            animate={{ opacity: 1, y: 0 }}
